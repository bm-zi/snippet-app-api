...
class PrivateSnippetApiTests(TestCase):
    ...
    def test_filter_by_tags(self):
        ...
        r1.tags.add(tag1)
        r2.tags.add(tag2)
        r3 = create_snippet(user=self.user, language_name='go')

        params = {'tags': f'{tag1.id},{tag2.id}'}
        res = self.client.get(SNIPPETS_URL, params)

        s1 = SnippetSerializer(r1)
        s2 = SnippetSerializer(r2)
        s3 = SnippetSerializer(r3)
